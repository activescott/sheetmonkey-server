...
describe('DB', function () {
  ...
  describe('apiTokens', function () {
    ...
    beforeEach(function () {
      return db.deleteAllApiTokens()
    })

    it('should add and get a token', function () {
      const plugin = `https://blah.blah/${userID}.json`
      const token = `giberish_token_${randomUserID()}`
      return db.addApiTokenForPluginUser(plugin, userID, token, token, 123456).then(gotToken => {
        expect(gotToken, 'gotToken').to.have.property('access_token', token)
        return db.getApiTokenForPluginUser(plugin, userID).then(gotToken2 => {
          return expect(gotToken2, 'gotToken2').to.have.property('access_token', token)
        })
      })
    })
  })
