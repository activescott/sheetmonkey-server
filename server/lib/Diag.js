...
class Diag {
  ...
  log () {
    ...
    args = [`${this.timestamp()} ${this._prefix}:`].concat(args)
  ...
  assert () {
    ...
    args = sanitizeArgs(args)
    args = args.concat([`${this.timestamp()} ${this._prefix}:`])
...
function sanitizeArgs (args) {
  ...
  for (let sourceArg of args) {
    let argCopy
    if (typeof sourceArg === 'undefined') { argCopy = undefined } else if (sourceArg) {
